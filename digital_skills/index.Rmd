---
pagetitle: "Digital Skills Services in Trafford"
lang: "en-GB"
output: 
  html_document:
    css: styles.css
...
df <- read_csv("data/digital_skills_services.csv") %>%
  mutate(Category = fct_relevel(as_factor(Category),
                             level = c("Community hub", "Library", "Charity/community group", "Education establishment", "Business", "Other")),
         `Targeted?` = fct_relevel(as_factor(`Targeted?`),
                             level = c("Inclusive", "Elderley", "Families", "Unemployed", "Women", "Students", "Young people", "Customers", "Adults and young people with disabilities", "Area’s residents, Inclusive")),
         popup = str_c("<strong>", `Organisation name`, "</strong><br/>",
                        ...
                        #ifelse(is.na(Postcode),"",str_c("Postcode: ", Postcode))
                       ) %>% map(HTML))
...
map <- leaflet(sd, width = "100%", height = 350) %>%
  addTiles(urlTemplate = "https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png", attribution = '&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors &copy; <a href="https://carto.com/attributions">CARTO</a> | <a href="https://www.ons.gov.uk/methodology/geography/licences">Contains OS data © Crown copyright and database right (2021)</a>', options = tileOptions(minZoom = 11, maxZoom = 18)) %>%
...
div(class = "container-fluid",
    h1("Digital Skills Services in Trafford", style = "padding-top: 25px; margin-bottom: 20px;"),
    div(class = "row",
        ...
        div(class = "col-xs-6 col-sm-3 col-md-3 col-lg-3", 
            list(
              ...
              br(),
              # WiFi
            filter_checkbox(id = "WiFi", label = "WiFi", sharedData = sd, group = ~ifelse(WiFi == "x", "Avalilable", NA), inline = TRUE),
            br(),
              # Device lending scheme
            filter_checkbox(id = "Device lending scheme", label = "Device lending scheme", sharedData = sd, group = ~ifelse(`Device lending scheme` == "x", "Avalilable", NA), inline = TRUE),
            br(),
              # Training and/or support
            filter_checkbox(id = "Training and/or support", label = "Training and/or support", sharedData = sd, group = ~ifelse(`Training and/or support` == "x", "Avalilable", NA), inline = TRUE)
            )),
        div(class = "col-xs-6 col-sm-3 col-md-3 col-lg-3", 
            list(
              # Targeted?
              filter_checkbox(id = "Targeted?", label = "Targeted?", sharedData = sd, group = ~`Targeted?`),
              br(),
              # Access
            filter_checkbox(id = "Access", label = "Access", sharedData = sd, group = ~ifelse(Access == "x", "Avalilable", NA), inline = TRUE)
              ))),
    br(),
    ...
    div(class = "row",
      div(class = "col-sm-12", 
          p("O2 offers free WiFi at ", a("multiple locations across Trafford", href = "https://www.o2wifi.co.uk/hotspot/", target = "_blank")),
      br()
      )
   ))
```
