# Trafford's 2023 wards and localities#
...
localities <- st_read("data/localities_2023.geojson")
...
map <- leaflet(height = "100%", width = "100%") %>% 
  ...
  addPolygons(data = wards, fillColor = "#CCCCCC", weight = 3, fillOpacity = 0.05, opacity = 1, color = "#212121", label = ~area_name, group = "Wards 2023") %>%
  
  addLabelOnlyMarkers(data = wards, lng = ~lon, lat = ~lat, group = "Wards 2023", label = ~as.character(area_name), 
  ...
  addPolygons(data = localities, fillColor = "#bd0026", weight = 7, fillOpacity = 0, opacity = 1, color = "#2A81CB", label = ~area_name, group = "Localities 2023", highlightOptions = highlightOptions(color = "yellow", weight = 3, bringToFront = TRUE)) %>% 
  addLabelOnlyMarkers(data = localities, lng = ~lon, lat = ~lat, group = "Localities 2023", label = ~as.character(area_name), 
                      labelOptions = labelOptions(noHide = T, textOnly = T, direction = "top", offset = c(0,15),
                                                  style = list(
                                                    "color"="#2A81CB",
                                                    "font-size" = "15px",
  ...
  addControl(paste0("<h1>Trafford's 2023 wards and localities</h1>"), position = 'topright',  className = "map-title") %>% 
  addLayersControl(
    #overlayGroups = c("Trafford wards", "Trafford option A","Trafford option B","LGBCE draft","LGBCE final"),
    overlayGroups = c("Wards 2023", "Localities 2023"),
